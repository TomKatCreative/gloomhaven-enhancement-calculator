...
void main() {
  ...
  group('PerksSection', () {
    testWidgets('renders "Perks:" header', (tester) async {
      final perks = TestData.createPerkList(count: 2);
      final character = TestData.createCharacter();
    ...
    testWidgets('displays correct selected perk count', (tester) async {
      ...
      final model = await setupModelWithPerks(
        ...
        perkData: [
          (perkId: perks[0].perkId, isSelected: true),
          (perkId: perks[1].perkId, isSelected: true),
          (perkId: perks[2].perkId, isSelected: false),
        ],
        isEditMode: false,
      );
      ...
      // Find the selected count text (numOfSelectedPerks = 2)
      final countText = find.byWidgetPredicate((widget) {
        if (widget is Text && widget.data == '2') {
          // Only match the count text, not other '2' texts
          return true;
        }
        return false;
      });
    ...
    testWidgets('displays correct maximum perk count', (tester) async {
      final perks = TestData.createPerkList(count: 3);
      // Level 1, 0 checkmarks, 0 retirements → max perks = 0
      final character = TestData.createCharacter();
      ...
      final expectedMax = Character.maximumPerks(character);
      expect(find.text('$expectedMax'), findsAtLeastNWidgets(1));
    });

    testWidgets('selected count turns red when exceeding max', (tester) async {
      final perks = TestData.createPerkList(count: 3);
      // Level 1, 0 checkmarks, 0 retirements → max perks = 0, selected = 3
      final character = TestData.createCharacter();
      ...
      // Find the '3' text showing selected count with red color
      final redText = find.byWidgetPredicate((widget) {
        if (widget is Text && widget.data == '3') {
    ...
    testWidgets('selected count is default color when within max', (
      tester,
    ) async {
      final perks = TestData.createPerkList(count: 3);
      // Level 9 (500 XP) + 2 retirements → max perks = 8 + 2 = 10
      final character = TestData.createCharacter(
        xp: 500,
      ...
      expect(redText, findsNothing);
    });

    testWidgets('perk count updates after toggling checkbox', (tester) async {
      // BUG-CATCHER: Verifies the count text changes in the widget tree
      // after toggling a perk checkbox. Uses Consumer to ensure PerksSection
      // rebuilds when the model notifies (matching real app behavior).
      final perks = TestData.createPerkList(count: 3);
      final character = TestData.createCharacter();
      ...
      await tester.pumpWidget(
        buildTestWidget(
          ...
          child: Consumer<CharactersModel>(
            builder: (context, _, _) => PerksSection(character: character),
          ),
        ),
      );

      // Initially 0 selected - verify the count
      ...
      expect(character.numOfSelectedPerks, equals(1));

      // Verify the '1' text appears in the rebuilt widget tree
